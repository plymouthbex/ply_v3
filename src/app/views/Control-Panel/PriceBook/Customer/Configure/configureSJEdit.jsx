...
const ConfigureSJEdit = () => {
  ...
  console.log("🚀 ~ configureSJEdit ~ State:", State);
  const { user } = useAuth();
  console.log("🚀 ~ configureSJEdit ~ user:", user);
...
const [editedRows, setEditedRows] = useState({});
  ...
  console.log("🚀 ~ configureSJEdit ~ addedRows:", addedRows);
...
const filteredSelectedItems = useMemo(() => {
  const getRowsSet = new Set(
    getRows.map((item) => item.RecordID)
  );

  return addedRows.filter(
    (selectedItem) =>
      !getRowsSet.has(selectedItem.RecordID)
  );
}, [getRows, addedRows]);
...
const processRowUpdate = (newRow, oldRow) => {
  ...
  const CustomToolBar = () => {
    return (
      ...
      >
        <Typography>Total Item Count: {data.CompanyCode=="SJ"?data.PriceSheetItemCount:data.PriceListItemCount}</Typography>
        <GridToolbarQuickFilter />
      </GridToolbarContainer>
    );
  };
...
//   }}
  slotProps={{
    ...
    row: {
      draggable: true,
      onDragStart: (event) => {
        dragRowId.current = event.currentTarget.getAttribute("data-id");
...
export default ConfigureSJEdit;
