...
const ItemAttributesEdit = () => {
  ...
  useEffect(() => {
    dispatch(getPriceListItemGet({ RecordID:state.RecordID, id: state.headerID,type:"Q" }));
  }, []);
  ...
  const itemSaveFn = async (values) => {
    const data = {
      ...
      priceListID: "0",
      quotationRecordID:  state.headerID.toString(),
      filterType: "Q",
      itemNo: itemNumber,
      printSequence: values.sequence,
      printItem: values.printItem ? "1" : "0",
      comment: values.comments,
      quotationPrice:values.quotePrice,
    };
    ...
    <Container>
      <Formik
        initialValues={{
          ...
          quotePrice: priceListAttData.QI_QUOTATIONPRICE,
